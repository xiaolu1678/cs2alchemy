                  ...
                  <TableBody>
                    {visibleInventory.map((item) => (
                      <TableRow key={item.id} className={cx("transition-colors hover:bg-white [&>td]:whitespace-nowrap", selectedIds.includes(item.id) && "bg-indigo-50/70 hover:bg-indigo-50")}> 
                        ...
                        <TableCell className="pl-6 pr-4 align-middle">{isRowEditable(item.id) ? <Input className="h-10 w-[150px] rounded-xl bg-white text-center" type="date" value={item.date ?? ""} onChange={(e) => updateInventoryField(item, "date", e.target.value)} /> : formatInventoryDate(item.date, Boolean(inventoryFilters.date))}</TableCell>
